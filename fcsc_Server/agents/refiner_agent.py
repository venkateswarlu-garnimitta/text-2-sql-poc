        ...
        try:
            ...
            if decomposition:
                ...
                prompt = f"""
                        ...
                        You are given:
                        1. A database schema (case-sensitive) in SQLite/PostgreSQL dialect:
                        {schema_string}
                        ...
                        Strict SQL generation guidelines:
                        - Output **only** the SQL query as plain text. DO NOT include markdown (no ```sql or ```).
                        - Always match column/table names **exactly as given** in the schema, including proper **case** (e.g., "Quarter", not "quarter").
                        - Use double quotes around column/table names that are case-sensitive or contain spaces/special characters.
                        - Use table aliases (`r`, `e`, etc.) consistently when multiple tables are involved.
                        - Use `GROUP BY` for any column in the `SELECT` that is not aggregated.
                        - Prefer **explicit JOINs** for multi-table queries.
                        - Use **CTEs (WITH)** only if the logic is complex or improves readability.
                                - Avoid subqueries unless necessary.
                                - Ensure the final query is directly executable in a SQL engine (SQLite/PostgreSQL).
            ...
            Your task is to generate a **valid, optimized SQL query** that accurately answers the user's question using the provided schema.
