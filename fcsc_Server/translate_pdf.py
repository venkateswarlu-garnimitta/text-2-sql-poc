...
class ArabicPDFReportGenerator:
    """
    Class responsible for generating Arabic PDF reports containing query information,
    SQL query, data results, and a summary with RTL support.
    """
    ...
    def _register_arabic_font():
        ...
        try:
            ...
            print("Falling back to default font. Arabic text may not display correctly.")
            return False
    
    @staticmethod
    def _format_currency(value, for_table_display=False, for_chart_axis_label=False, for_chart_hover=False) -> str:
        """
        Formats a numerical value into international standard (K, M, B) with optional 'درهم' suffix.
        ...
        if abs_value >= 1_000_000_000:
            suffix = "ب" # مليار - Billion
            divisor = 1_000_000_000
        elif abs_value >= 1_000_000:
            suffix = "م" # مليون - Million
            divisor = 1_000_000
        elif abs_value >= 1_000:
            suffix = "ألف" # ألف - Thousand
            divisor = 1_000

        formatted_num = numeric_value / divisor

        if for_table_display:
            # For table display: 1ألف, 22م (no درهم)
            ...
            # For chart axis labels: 1.5م, 250ألف (no درهم, concise)
            if formatted_num == int(formatted_num):
                return f"{int(formatted_num):,}{suffix}"
            else:
                return f"{formatted_num:,.1f}{suffix}" # .1f for conciseness on axis
        elif for_chart_hover:
            # For chart hover: 1,234,567.89 درهم (full precision with درهم)
            return f"{numeric_value:,.2f} درهم"
        else:
            # Default for general use, similar to table but can be adjusted
            if formatted_num == int(formatted_num):
                return f"{int(formatted_num):,}{suffix} درهم"
            else:
                return f"{formatted_num:,.2f}{suffix} درهم"

    @staticmethod
    def _first_page_only(canvas, doc):
        """
        Custom header and footer for only the first page of the PDF.
        Includes left and right logos, and a centered title in Arabic.
        """
        canvas.saveState()
        ...
        title_style = ParagraphStyle(
            ...
            fontName='NotoNaskhArabic' if ArabicPDFReportGenerator._register_arabic_font() else 'Helvetica'
        )
        ...
        # Add left logo
        if left_logo_buffer:
            img = Image(left_logo_buffer, width=2.5*0.6*inch, height=0.6*inch)
        ...
        if right_logo_buffer:
            img = Image(right_logo_buffer, width=2.5*0.6*inch, height=0.6*inch)
            img_width, img_height = img.wrapOn(canvas, doc.width, doc.topMargin) 
            img.drawOn(canvas, doc.width + doc.leftMargin - img_width, header_content_y_start - img_height) 

        # Add title (تقرير استعلام NL2SQL) in the center
        title_text = "تقرير استعلام NL2SQL"
        ...
        footer_text = f"تم الإنشاء في: {timestamp}"
    ...
    def generate_report(
        ...
        """
        Generates a comprehensive Arabic PDF report with query details, summary, and visualizations.
        
        Args:
            query (str): The original natural language query in Arabic
            sql_query (str): The generated SQL query
            result_df (pd.DataFrame): The query result data
            summary (str): Markdown-formatted summary of the data analysis in Arabic
        ...
        has_arabic_font = ArabicPDFReportGenerator._register_arabic_font()
        ...
        # Define custom styles for Arabic text and add them to the stylesheet
        arabic_font = 'NotoNaskhArabic' if has_arabic_font else 'Helvetica'
        ...
        normal_style = ParagraphStyle(
            ...
            alignment=TA_RIGHT  # Right-to-left alignment for Arabic
        )
        ...
        code_style = ParagraphStyle(
            'ArabicCodeStyle', 
            parent=styles['Code'], 
            fontName='Courier',  # Keep monospace for SQL
            fontSize=9,
            leading=12,
            leftIndent=10,
            rightIndent=10,
            ...
            borderPadding=5,
            spaceAfter=10,
            alignment=TA_LEFT  # SQL is left-to-right
        )
        ...
        # Add Query Section
        elements.append(Paragraph("الاستعلام باللغة الطبيعية", styles['ArabicHeading2'])) 
        elements.append(Paragraph(query, styles['ArabicNormal'])) 
        
        # Add SQL Query Section
        elements.append(Paragraph("استعلام SQL المُولد", styles['ArabicHeading2'])) 
        formatted_sql = sqlparse.format(sql_query, reindent=True, keyword_case='upper')
        elements.append(Paragraph(formatted_sql, styles['ArabicCodeStyle'])) 
        ...
        for col in df_processed.select_dtypes(include=['object', 'string']).columns:
            ...
            if temp_numeric_series.notna().all(): # If all non-null values are convertible
                ...
                for val in df_processed[col].dropna().head(10): # Check first 10 non-null values
                    if isinstance(val, str) and ('درهم' in val or 'AED' in val or '€' in val or '$' in val or '£' in val or '₹' in val):
        ...
        # 1. Keywords in column names (Arabic and English)
        keyword_currency_cols = [
            col for col in numeric_cols_current
            if any(keyword in col.lower() for keyword in ['amount', 'value', 'price', 'total', 'revenue', 'cost', 'spend', 'budget', 'sales', 'profit', 'aed', 'مبلغ', 'قيمة', 'سعر', 'إجمالي', 'إيراد', 'تكلفة', 'مصروف', 'ميزانية', 'مبيعات', 'ربح', 'درهم'])
        ]
        ...
        if df_processed is not None and not df_processed.empty:
            elements.append(Paragraph("نتائج الاستعلام", styles['ArabicHeading2'])) 
            elements.append(Paragraph(f"عدد الصفوف: {len(df_processed)} | عدد الأعمدة: {len(df_processed.columns)}", 
                                     styles['ArabicNormal'])) 
            
            preview_df = df_processed.head(10) # Use df_processed here
            
            # Create reversed table data for RTL display
            table_data = [list(reversed(preview_df.columns.tolist()))]  # Header row (reversed)
            for i, row in preview_df.iterrows():
                formatted_row = []
                for col_name, val in reversed(list(row.items())):  # Reverse the order
                    if col_name in currency_like_cols: # Use the identified currency columns
                        formatted_row.append(Paragraph(ArabicPDFReportGenerator._format_currency(val, for_table_display=True), styles['ArabicTableCellStyle']))
                    else:
                        formatted_row.append(Paragraph(str(val), styles['ArabicTableCellStyle']))
            ...
            if table_data:
                ...
                table_style = TableStyle([
                    ...
                    ('ALIGN', (0, 0), (-1, -1), 'CENTER'), # Content Center for Arabic
                ...
                if len(preview_df.columns) > 5:
                    elements.append(Paragraph("ملاحظة: المعاينة تُظهر أول 10 صفوف. قد يتم التفاف القيم داخل الأعمدة.", 
                                             styles['ArabicNote'])) 
            
            if summary:
                elements.append(Paragraph("ملخص تحليل البيانات", styles['ArabicHeading2'])) 
                
                def parse_arabic_markdown_to_flowables(md_text, styles):
                    flowables = []
                    # Handle Arabic markdown formatting
                    ...
                    for line in lines:
                        ...
                        if line.startswith('### '):
                            flowables.append(Paragraph(line[4:].strip(), styles['ArabicHeading3']))
                        elif line.startswith('## '):
                            flowables.append(Paragraph(line[3:].strip(), styles['ArabicHeading2']))
                        elif line.startswith('# '):
                            flowables.append(Paragraph(line[2:].strip(), styles['ArabicTitle']))
                        elif line.startswith('* ') or line.startswith('- '):
                            flowables.append(Paragraph(f'<bullet>&bull;</bullet>{line[2:].strip()}', styles['ArabicBulletStyle']))
                        else:
                            flowables.append(Paragraph(line, styles['ArabicSummaryStyle']))
                    return flowables
                
                summary_flowables = parse_arabic_markdown_to_flowables(summary, styles)
                elements.extend(summary_flowables)
            if not df_processed.empty: # Use df_processed here
                elements.append(PageBreak())
                elements.append(Paragraph("Visualizations", styles['ArabicHeading2'])) 
                ...
                for vis_type in vis_types:
                    try:
                        img_data = ArabicPDFReportGenerator._create_visualization_in_memory(
                        ...
                        if img_data:
                            title_para = Paragraph(f"{vis_type}", styles['ArabicHeading3'])
        ...
        first_page_template = PageTemplate(id='FirstPage', frames=[Frame(
            ...
            onPage=ArabicPDFReportGenerator._first_page_only)
        
        later_pages_template = PageTemplate(id='LaterPages', frames=[Frame(
            doc.leftMargin, doc.bottomMargin, doc.width, doc.height)],
            onPage=ArabicPDFReportGenerator._later_pages)
            ...
            if vis_type == "Bar Chart":
                ...
                if len(categorical_cols) > 0 and len(numeric_cols) > 0:
                    ...
                    for bar in bars:
                        ...
                        ax.annotate(ArabicPDFReportGenerator._format_currency(height, for_chart_axis_label=True),
                    ...
                    if y_col in currency_like_cols:
                        formatter = plt.FuncFormatter(lambda x, pos: ArabicPDFReportGenerator._format_currency(x, for_chart_axis_label=True))
                ...
                if len(numeric_cols) >= 2:
                    ...
                    if x_col in currency_like_cols:
                        formatter_x = plt.FuncFormatter(lambda x, pos: ArabicPDFReportGenerator._format_currency(x, for_chart_axis_label=True))
                        ax.xaxis.set_major_formatter(formatter_x)
                    
                    # Y-axis formatter: Use _format_currency if currency
                    if y_col in currency_like_cols:
                        formatter_y = plt.FuncFormatter(lambda y, pos: ArabicPDFReportGenerator._format_currency(y, for_chart_axis_label=True))
                ...
                if len(categorical_cols) > 0 and len(numeric_cols) > 0:
                    ...
                    for cat, val in zip(plot_df[x_col], plot_df[y_col]):
                        if y_col in currency_like_cols:
                            legend_labels.append(f"{cat} ({ArabicPDFReportGenerator._format_currency(val, for_chart_hover=True)})")
                        else:
...
if __name__ == "__main__":
    ...
    # Create sample dataframe with Arabic column names
    data = {
        'التاريخ': pd.date_range(start='2023-01-01', periods=10),
        'القسم': ['المبيعات', 'التسويق', 'تقنية المعلومات', 'الموارد البشرية', 'المالية', 'العمليات', 'المبيعات', 'التسويق', 'تقنية المعلومات', 'المالية'],
        'المبلغ': [500000, 350000, 250000, 150000, 600000, 450000, 700000, 200000, 350000, 550000],
        'الكمية': [5, 3, 7, 2, 6, 4, 8, 5, 4, 7],
        'الإيراد_2023': [1000000, 700000, 500000, 300000, 1200000, 900000, 1400000, 400000, 700000, 1100000],
        'التكلفة_2024': [300000, 200000, 150000, 80000, 400000, 250000, 450000, 120000, 200000, 350000],
        'الربح_الربع_الأول': [700000, 500000, 350000, 220000, 800000, 650000, 950000, 280000, 500000, 750000],
        'عدد_الموظفين': [50, 30, 25, 15, 60, 40, 70, 35, 28, 55]
    }
    
    df = pd.DataFrame(data)
    
    query = "أظهر لي الإيرادات حسب القسم"
    sql_query = "SELECT القسم, SUM(الإيراد) AS إجمالي_الإيراد FROM بيانات_المبيعات GROUP BY القسم ORDER BY إجمالي_الإيراد DESC"
    
    summary = """
    # تحليل الإيرادات حسب القسم
    
    تُظهر البيانات توزيع الإيرادات عبر الأقسام المختلفة:
    
    * **المبيعات** لديها أعلى مساهمة في الإيرادات بـ 2.4 مليون درهم (33% من الإجمالي).
    * **المالية** تليها بـ 1.75 مليون درهم (24% من الإجمالي).
    * **العمليات** تساهم بـ 0.9 مليون درهم (12% من الإجمالي).
    * **التسويق** يُحقق 1.1 مليون درهم (15% من الإجمالي).
    * **تقنية المعلومات** لديها 1.2 مليون درهم (16% من الإجمالي).
    
    يُظهر قسم المالية أعلى متوسط قيمة معاملة، بينما المبيعات لديها أعلى عدد معاملات.
    
    ### الرؤى الرئيسية
    
    - ثلاثة أقسام (المبيعات، المالية، تقنية المعلومات) تُمثل 73% من إجمالي الإيرادات.
    - هناك إمكانية للنمو في قسم الموارد البشرية، والذي لديه حالياً أقل مساهمة."""
    
    # Generate the PDF report
    output_file = "output/query_report.pdf"
    os.makedirs("output", exist_ok=True)
    
    try:
        result_file = ArabicPDFReportGenerator.generate_report(
            query=query,
            sql_query=sql_query,
            result_df=df,
            summary=summary,
        ...
        print(f"PDF report generated successfully: {result_file}")
    ...
    # Additional example with charts/visualizations
    try:
        ...
        revenues = df.groupby('القسم')['الإيراد_2023'].sum()
        
        # Create bar chart
        bars = ax.bar(departments, revenues, color=['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b'])
        
        # Customize chart
        ax.set_title('الإيرادات حسب القسم', fontsize=16, pad=20)
        ax.set_xlabel('القسم', fontsize=12)
        ax.set_ylabel('الإيراد (درهم)', fontsize=12)
        
        # Format y-axis values
        ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x/1000000:.1f}م'))
        ...
        for bar in bars:
            ...
            ax.text(bar.get_x() + bar.get_width()/2., height + height*0.01,
                   f'{height/1000000:.1f}م', ha='center', va='bottom')
    ...
    # Example of generating report with additional data analysis
    try:
...
* **إجمالي الإيرادات**: {df['الإيراد_2023'].sum():,.2f} درهم
* **متوسط الإيراد**: {df['الإيراد_2023'].mean():,.2f} درهم
* **أعلى إيراد**: {df['الإيراد_2023'].max():,.2f} درهم
* **أقل إيراد**: {df['الإيراد_2023'].min():,.2f} درهم

## توزيع الأقسام

تُظهر البيانات التوزيع التالي للإيرادات:

* **المبيعات**: يحقق أعلى إيراد بقيمة {df[df['القسم'] == 'المبيعات']['الإيراد_2023'].sum():,.0f} درهم
* **المالية**: ثاني أعلى قسم بقيمة {df[df['القسم'] == 'المالية']['الإيراد_2023'].sum():,.0f} درهم
* **تقنية المعلومات**: يساهم بـ {df[df['القسم'] == 'تقنية المعلومات']['الإيراد_2023'].sum():,.0f} درهم

### التوصيات

* **تطوير قسم التسويق**: يُنصح بزيادة الاستثمار في التسويق لتحسين الإيرادات
* **تحسين العمليات**: تحسين كفاءة العمليات يمكن أن يقلل التكاليف
* **التوسع في المبيعات**: القسم الأكثر ربحية يحتاج للتوسع

## المقاييس المالية

* **نسبة الربح الإجمالي**: {((df['الإيراد_2023'].sum() - df['التكلفة_2024'].sum()) / df['الإيراد_2023'].sum() * 100):.1f}%
* **متوسط عدد الموظفين**: {df['عدد_الموظفين'].mean():.0f} موظف
* **الإيراد لكل موظف**: {(df['الإيراد_2023'].sum() / df['عدد_الموظفين'].sum()):,.0f} درهم
        """
        
        # Generate enhanced report
        enhanced_output = "output/enhanced_arabic_report.pdf"
        
        enhanced_result = ArabicPDFReportGenerator.generate_report(
            query="تحليل شامل للأداء المالي والتشغيلي للأقسام المختلفة",
            sql_query="""
SELECT  * from all_data;
        ...
        print(f"Enhanced PDF report generated: {enhanced_result}")
    ...
    if os.path.exists("output/arabic_report.pdf"):
        print(f"- arabic_report.pdf ({os.path.getsize('output/arabic_report.pdf')} bytes)")
    if os.path.exists("output/enhanced_arabic_report.pdf"):
        print(f"- enhanced_arabic_report.pdf ({os.path.getsize('output/enhanced_arabic_report.pdf')} bytes)")
    if os.path.exists("output/revenue_chart.png"):
        print(f"- revenue_chart.png ({os.path.getsize('output/revenue_chart.png')} bytes)")